...
// MIDDLEWARE
app.use(express.static("public", { // 12. Serve files from /public folder
...
if (isProd && !process.env.SESSION_SECRET) { // 17. Ensure session secret exists in production
  throw new Error("SESSION_SECRET is required in production");
}
app.use(session({ // 18. Enable sessions with MongoDB storage
...
app.use((req, res, next) => { // 19. Run for every request before routes
...
// ROUTES
app.get("/", (req, res) => { // 20. Home page route
...
// ERROR HANDLING
app.use((req, res) => { // 23. 404 handler - runs if no routes matched
  res.status(404).render("404", { title: "Not Found" });
});
app.use((err, req, res, next) => { // 24. Global error handler - 4 parameters required
  console.error(err);
