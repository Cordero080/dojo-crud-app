...
async function getChartData(ownerId) {
  ...
  const alive = await Form.find({ deletedAt: null, owner: ownerId }).select('rankType rankNumber');
...
router.get('/forms/new', requireAuth, async (req, res) => {
  try {
    ...
    res.render('new', { title: 'Add New Martial Arts Form', error: null, errors: {}, formData: {}, ...data });
  } catch (e) {
    ...
    res.render('new', {
      ...
      errors: {}, formData: {},
      availableNames: [], chartLabels: [], chartCounts: [],
      kyuRequirements: {}, danRequirements: {}, kyuChipMap: {},
    });
  }
});
...
router.post('/forms', requireAuth, async (req, res, next) => {
  ...
  const renderNew = async ({ error = null, errors = {}, formData = req.body, status = 400 } = {}) => {
    ...
    return res.status(status).render('new', { title: 'Add New Martial Arts Form', error, errors, formData, ...data });
  };

  try {
    const {
      name = '', rankType, rankNumber,
      beltColor = '', category = 'Kata',
      description = '', referenceUrl = '', learned,
    } = req.body;
    ...
    const exists = await Form.exists({
      owner: ownerId, name: doc.name, rankType: doc.rankType, rankNumber: doc.rankNumber, deletedAt: null,
    });
    if (exists) return renderNew({ error: 'That form already exists for this rank.' });
...
router.get('/forms/:id/edit', requireAuth, async (req, res) => {
  try {
    ...
    res.render('forms/edit', { title: `Edit: ${form.name}`, form, error: null, errors: {}, formData: {} });
  } catch {
